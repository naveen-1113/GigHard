...
class BSHighlightableTextView: UITextView {
    ...
    private var contenOffset = NSRange()
    ...
    override func draw(_ rect: CGRect) {
        if let textView = getPersistedData() {
           ...
           // self.scrollRangeToVisible(textView.highlightedRanges[0])
    ...
    private func hightlightText() {
        ...
        if attributed.length > 0 {
            ...
            print(highlightedRanges)
            for range in highlightedRanges {
                let attributes = attributed.attributes(at: range.location, effectiveRange: nil)
                if ((attributes as AnyObject) .value(forKey: NSAttributedString.Key.backgroundColor.rawValue) != nil) {
            ...
            self.selectedRange = self.contenOffset
            self.persist()
            self.highlightedRanges.removeAll()
        }
    }
    ...
    @objc func selectHighlightArea() {
        ...
        self.contenOffset = self.selectedRange
        ...
        if indexes.count == 1 {
            ...
            newRange = NSRange.multipleJoin(ranges: matchedRanges)
...
//        self.perform(#selector(test(range:)), with: self, afterDelay: 0.1)
//        DispatchQueue.main.asyncAfter(deadline: .now() + 0.0) {
//            self.test(range: newRange)
//        }
//        self.scrollRangeToVisible(newRange)
//        newRange = NSRange()
    }
    
    @objc func test(range: NSRange) {
